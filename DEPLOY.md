...
4. 添加读取消息资源、发送消息、上传/发送文件等机器人所需权限，并发布应用版本给企业管理员审批。
```
...
文件上传消息不会立即创建报价任务，只会加入当前会话；收到“帮我报价”后才会创建任务。任务完成后，机器人会在飞书会话内发送报告内容，并把 Excel 成本表作为飞书文件发回；网页链接只作为文件发送失败时的备用下载方式。
